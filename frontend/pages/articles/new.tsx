...
const NewDiscussion = () => {
    ...
    const submitNewArticle = async (event: FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.post('http://localhost:8080/api/article', articleData);

            if (response.status === 200) {
                window.alert("Article submitted successfully!")
                console.log("Article submitted successfully!", response.data);
            } else {
                window.alert("Article submitted error!")
                console.error("Failed to submit the article:", response.data);
            }
        } catch (error) {
